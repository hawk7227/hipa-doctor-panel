...
// ============================================================================
// INITIAL STATE FACTORIES — matches @/types/bugsy exactly
// ============================================================================

const getInitialContext = (): InterviewContext => ({
  user_id: '', user_name: '', user_role: 'provider', session_id: '',
  page_url: '', page_name: '', browser: '',
  screen_size: { width: 0, height: 0 }, timestamp: new Date().toISOString(),
});

const getInitialRecording = (): RecordingData => ({
  video_url: null, duration_seconds: 0, file_size_bytes: 0,
});

const getInitialTranscript = (): TranscriptData => ({
  full_text: '', segments: [], keywords_found: [],
});

const getInitialAnalysis = (): AnalysisResult => ({
  problem_identified: '', elements_involved: [], components_identified: [],
  files_identified: [], api_endpoints_identified: [], pattern_matches: [], similar_bugs: [],
});

// GapTracking: 4 fields only — expected_behavior, error_message, frequency, worked_before
const getInitialGaps = (): GapTracking => ({
  expected_behavior: 'unknown', error_message: 'unknown', frequency: 'unknown', worked_before: 'unknown',
});

const getInitialAnswers = (): DoctorAnswers => ({
  reflection_confirmed: false, corrections: [], expected_behavior: [],
  error_message: null, frequency: null, worked_before: null, additional_notes: '',
});

const getInitialConfidence = (): ConfidenceData => ({
  score: 0,
  breakdown: { problem_clarity: 0, location_clarity: 0, steps_clarity: 0, expected_clarity: 0, context_clarity: 0, pattern_bonus: 0, total: 0 },
  minimum_required: BUGSY_CONFIG.MINIMUM_CONFIDENCE_TO_SUBMIT,
  ready_to_submit: false,
});

// ============================================================================
// HELPER: safely stringify any error value to a string
// ============================================================================
function errorToString(val: unknown): string {
  if (typeof val === 'string') return val;
  if (val instanceof Error) return val.message;
  if (val && typeof val === 'object') {
    // Supabase errors often have .message or .details
  ...
  return String(val || 'Unknown error');
}
...
export function useBugsyState(): UseBugsyStateReturn {
  ...
  const recalculateConfidence = useCallback(() => {
    const breakdown = calculateConfidence({
      transcript: transcript.full_text, problemIdentified: !!analysis.problem_identified,
      clicks: interactions, markers, pageUrl: context.page_url,
      elementsIdentified: analysis.elements_involved, componentsIdentified: analysis.components_identified,
      transcriptSegments: transcript.segments, reflectionConfirmed: answers.reflection_confirmed,
      expectedBehaviorKnown: gaps.expected_behavior, errorMessageKnown: gaps.error_message,
      frequencyKnown: gaps.frequency === 'known', workedBeforeKnown: gaps.worked_before === 'known',
  ...
  const initializeContext = useCallback((userId: string, userName: string, userRole: 'provider' | 'assistant') => {
    setContext(prev => ({
      ...prev, user_id: userId, user_name: userName, user_role: userRole,
  ...
  const answerQuestion = useCallback((key: string, answer: string | string[] | null) => {
    setAnswersState(prev => {
      ...
      else if (key === 'frequency') u.frequency = answer as DoctorAnswers['frequency'];
      else if (key === 'worked_before') u.worked_before = answer as DoctorAnswers['worked_before'];
      return u;
    });
  }, []);
  ...
  const submit = useCallback(async (): Promise<{ success: boolean; reportId?: string; error?: string }> => {
    ...
    try {
      const { priority, signals } = inferPriority({
        ...
        pageUrl: context.page_url, isClinicHours: isClinicHours(),
      });

      // recording_url is a blob URL — skip it for JSON submission (video not uploaded to storage yet)
      const reportData: CreateBugReportRequest = {
        ...
        recording_url: recording.video_url && !recording.video_url.startsWith('blob:') ? recording.video_url : undefined,
        ...
        bugsy_interview_data: { phase: 'success', context, recording: { ...recording, video_url: null }, transcript, interactions, markers, analysis, gaps, answers, confidence },
      ...
      const response = await fetch('/api/bugsy/submit', {
        ...
        body: JSON.stringify({ report: reportData, priority, priority_signals: signals }),
      });

      // Handle non-JSON responses (500 errors, HTML pages, etc.)
      ...
      if (result.success && result.data?.id) {
        ...
        // FIX: result.error might be an object — safely convert to string
        throw new Error(errorToString(result.error) || 'Failed to submit bug report');
      }
    } catch (err) {
      const msg = errorToString(err);
      console.error('Bugsy submit error:', msg);
      setError(msg);
      setPhase('error');
