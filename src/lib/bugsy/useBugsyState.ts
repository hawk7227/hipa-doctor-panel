...
export function useBugsyState(): UseBugsyStateReturn {
  ...
  const submit = useCallback(async (): Promise<{ success: boolean; reportId?: string; error?: string }> => {
    ...
    try {
      ...
      // Convert recording blob to base64 to send to API for server-side upload
      let videoBase64: string | null = null;
      if (recording.video_url && recording.video_url.startsWith('blob:')) {
        try {
          console.log('Bugsy: Converting recording to base64 for upload...');
          ...
          console.log('Bugsy: Video encoded, size:', Math.round(videoBase64.length / 1024), 'KB');
        } catch (encodeErr) {
          console.error('Bugsy: Failed to encode video:', encodeErr);
        }
      }

      const uploadedVideoUrl: string | null = null;
