...
export function useBugsyState(): UseBugsyStateReturn {
  ...
  const submit = useCallback(async (): Promise<{ success: boolean; reportId?: string; error?: string }> => {
    ...
    try {
      ...
      // Upload recording video to Supabase Storage if it's a blob URL
      ...
      if (recording.video_url && recording.video_url.startsWith('blob:')) {
        try {
          ...
          // Continue with submit even if upload fails — report still has transcript
        }
      } else if (recording.video_url) {
        // Already a real URL (not blob)
        uploadedVideoUrl = recording.video_url;
      }
