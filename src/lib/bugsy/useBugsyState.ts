...
// ============================================================================
// INITIAL STATE FACTORIES — must match types in @/types/bugsy exactly
// ============================================================================

// InterviewContext: user_id, user_name, user_role, session_id, page_url, page_name, browser, screen_size, timestamp
// Optional fields (page_elements, recent_actions, known_issues) omitted — they're optional
...
// RecordingData: video_url, duration_seconds, file_size_bytes
...
// TranscriptData: full_text, segments, keywords_found
...
// AnalysisResult: problem_identified, elements_involved, components_identified, files_identified, api_endpoints_identified, pattern_matches, similar_bugs
...
// GapTracking: expected_behavior, error_message, frequency, worked_before (4 fields only)
...
// DoctorAnswers: reflection_confirmed, corrections, expected_behavior, error_message, frequency, worked_before, additional_notes
...
// ConfidenceData: score, breakdown, minimum_required, ready_to_submit
...
export function useBugsyState(): UseBugsyStateReturn {
  ...
  // ── Set analysis ──
  const setAnalysis = useCallback((a: AnalysisResult) => {
    setAnalysisState(a);
    phaseHistoryRef.current.push(phase);
    setPhase('reflection');
  }, [phase]);
  ...
  const setAnswers = useCallback((newAnswers: Partial<DoctorAnswers>) => {
    ...
    if (phase === 'clarifying' && getRequiredQuestions(newGaps).length === 0) {
      phaseHistoryRef.current.push(phase);
  ...
  const goBack = useCallback(() => {
    const p = phaseHistoryRef.current.pop();
    if (p) setPhase(p);
  }, []);
  ...
  return {
    ...
    initializeContext, startRecording, stopRecording, setAnalysis, confirmReflection,
    answerQuestion, setAnswers, goBack, submit, reset, setError,
  };
}
