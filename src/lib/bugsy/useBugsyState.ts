...
const getInitialContext = (): InterviewContext => ({ user_id: '', user_name: '', user_role: 'provider', page_url: '', page_name: '', browser: '', screen_size: { width: 0, height: 0 }, timestamp: new Date().toISOString() });
...
export function useBugsyState(): UseBugsyStateReturn {
  ...
  const initializeContext = useCallback((userId: string, userName: string, userRole: 'provider' | 'assistant') => {
    setContext(prev => ({ ...prev, user_id: userId, user_name: userName, user_role: userRole, page_url: typeof window !== 'undefined' ? window.location.pathname : '', page_name: typeof window !== 'undefined' ? getPageNameFromUrl(window.location.pathname) : '', browser: typeof window !== 'undefined' ? getBrowserInfo() : '', screen_size: typeof window !== 'undefined' ? getScreenSize() : { width: 0, height: 0 }, timestamp: new Date().toISOString() }));
  }, []);
