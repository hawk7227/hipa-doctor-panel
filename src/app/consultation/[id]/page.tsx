...
export default function ConsultationPageRoute({ params: routeParams }: { params?: { id?: string } }) {
  const hookParams = useParams()
  const appointmentId = (routeParams?.id || hookParams?.id as string) || ''
    ...
    <div className="relative w-full" style={{ height: height || '100%', background: '#000', minHeight: 200 }}>
      ...
      {appointment?.dailyco_meeting_url && (
        <DailyMeetingEmbed
          ...
          currentUser={doctor ? { email: doctor.email, name: `${doctor.first_name} ${doctor.last_name}` } : null}
        />
      )}
