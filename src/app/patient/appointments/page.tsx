...
export default function PatientAppointments() {
  ...
  const getStatusColor = (status: string) => {
    const colors = {
      ...
      cancelled: 'bg-[#151D28] text-gray-800'
    }
    return colors[status as keyof typeof colors] || 'bg-[#151D28] text-gray-800'
  }
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="animate-pulse">
            ...
            <div className="space-y-4">
              {[1, 2, 3].map((i) => (
                <div key={i} className="bg-[#151D28] rounded-lg p-6">
    ...
    <div className="min-h-screen bg-[#111820] py-8">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="mb-8">
          ...
          <p className="mt-2 text-[#4A5568]">Manage your appointments and upload medical records</p>
        </div>

        {appointments.length === 0 ? (
          <div className="bg-[#151D28] rounded-lg shadow p-8 text-center">
            <Calendar className="h-12 w-12 text-[#E8ECF1] mx-auto mb-4" />
            <h3 className="text-lg font-medium text-gray-900 mb-2">No appointments found</h3>
            <p className="text-[#7B8CA3]">You haven't booked any appointments yet.</p>
          </div>
        ) : (
          <div className="space-y-6">
            {appointments.map((appointment) => (
              <div key={appointment.id} className="bg-[#151D28] rounded-lg shadow">
                <div className="p-6">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div className="flex items-center gap-2">
                      <Clock className="h-4 w-4 text-[#7B8CA3]" />
                      <span className="text-sm text-[#4A5568]">
                  ...
                  {appointment.zoom_meeting_url && appointment.visit_type === 'video' && (
                    <div className="mb-4">
                      <a
                        ...
                        className="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-md text-[#E8ECF1] bg-blue-600 hover:bg-blue-700"
                      >
                        Join Zoom Meeting
                      </a>
