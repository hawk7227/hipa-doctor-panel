...
import { 
  Search, Phone, MessageSquare, Send, Clock, PhoneCall, 
  Play, Pause, Download, X, Mic, MicOff,
  PhoneOff, User, ChevronDown, RefreshCw, ArrowLeft
} from 'lucide-react'
...
export default function CommunicationPage() {
  ...
  const filteredPatients = patients.filter(p =>
    !patientSearch || `${p.first_name} ${p.last_name}`.toLowerCase().includes(patientSearch.toLowerCase()) ||
        ...
        <div className="flex items-center justify-between px-4 py-2.5">
          ...
          <div ref={patientDropdownRef} className="relative">
            <button
              ...
              className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-[#0d2626] border border-[#1a3d3d] hover:border-teal-500/50 transition-colors text-sm"
            >
              {selectedPatient ? (
                <>
                  ...
                  <button
                    onClick={(e) => { e.stopPropagation(); setSelectedPatient(null); setPhoneNumber(''); setSmsTo('') }}
            ...
            {showPatientDropdown && (
              <div className="absolute right-0 top-full mt-1 w-72 bg-[#0d2626] border border-[#1a3d3d] rounded-lg shadow-2xl z-50 overflow-hidden">
                <div className="p-2 border-b border-[#1a3d3d]">
                  <input
                    type="text"
                    placeholder="Search patients..."
                    value={patientSearch}
                    onChange={(e) => setPatientSearch(e.target.value)}
                    className="w-full bg-[#164e4e] border border-[#1a5a5a] rounded px-3 py-1.5 text-sm text-white placeholder-gray-500 focus:outline-none focus:border-teal-500"
                    autoFocus
                  />
                </div>
                <div className="max-h-60 overflow-y-auto">
                  {loadingPatients ? (
                    ...
                    <button
                      ...
                      className={`w-full text-left px-3 py-2 hover:bg-[#164e4e] transition-colors text-sm border-b border-[#1a3d3d]/40 ${selectedPatient?.id === p.id ? 'bg-[#164e4e] border-l-2 border-l-teal-500' : ''}`}
                    >
