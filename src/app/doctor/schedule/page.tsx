  ...
  if (loading) {
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
        <div className="w-12 h-12 border-b-2 border-[#00D4AA] rounded-full animate-spin"></div>
      </div>
    )
  }
  ...
  return (
    <>
      ...
      <style>{`
        ...
        /* ============================================ */
        /* ENTERPRISE CALENDAR GRID STYLES */
        /* ============================================ */
        ...
        /* Hide sparkle emoji on available slots */
        ...
        @keyframes timeIndicatorPulse {
          ...
          50% { box-shadow: 0 0 10px rgba(0, 212, 170, 0.5); }
        }
      ...
      <div className="availability-page" style={{ height: '100vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        {/* ============================================ */}
        {/* Enterprise Schedule Header */}
        <div style={{ flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 16px', borderBottom: '1px solid #1E2A3A', background: '#111820', flexWrap: 'wrap', gap: '8px' }}>
          {/* Left: Title + View Tabs */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <h1 style={{ margin: 0, fontSize: '18px', fontWeight: 700, color: '#E8ECF1' }}>Schedule</h1>
            <div style={{ display: 'flex', background: '#151D28', borderRadius: '10px', padding: '2px', border: '1px solid #1E2A3A' }}>
              {(['week', 'month'] as const).map(v => (
                <button key={v} onClick={() => setCalendarViewType(v)} style={{
                  padding: '5px 14px', borderRadius: '8px', border: 'none', fontSize: '12px', fontWeight: 600, cursor: 'pointer',
                  background: calendarViewType === v ? '#1E2A3A' : 'transparent', color: calendarViewType === v ? '#E8ECF1' : '#7B8CA3',
                }}>{v.charAt(0).toUpperCase() + v.slice(1)}</button>
              ))}
              <button onClick={() => setViewType(viewType === 'calendar' ? 'list' : 'calendar')} style={{
                padding: '5px 14px', borderRadius: '8px', border: 'none', fontSize: '12px', fontWeight: 600, cursor: 'pointer',
                background: viewType === 'list' ? '#1E2A3A' : 'transparent', color: viewType === 'list' ? '#E8ECF1' : '#7B8CA3',
              }}>List</button>
            </div>
          </div>
          {/* Center: Legend */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
            <span style={{ display: 'flex', alignItems: 'center', gap: '4px', fontSize: '11px', color: '#7B8CA3' }}>
          ...
          {/* Right: Date nav + Today */}
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            <button onClick={() => navigateCalendar('prev')} style={{ padding: '4px 8px', background: 'transparent', border: '1px solid #1E2A3A', borderRadius: '6px', color: '#7B8CA3', cursor: 'pointer', fontSize: '12px' }}>←</button>
            <span style={{ fontSize: '12px', color: '#E8ECF1', fontWeight: 500, minWidth: '120px', textAlign: 'center' }}>
              {currentPhoenixTime.dateStr || currentDate.toLocaleDateString('en-US', { month: 'short', day: 'numeric', year: 'numeric' })}
            </span>
            <button onClick={() => navigateCalendar('next')} style={{ padding: '4px 8px', background: 'transparent', border: '1px solid #1E2A3A', borderRadius: '6px', color: '#7B8CA3', cursor: 'pointer', fontSize: '12px' }}>→</button>
            <button onClick={goToToday} style={{ padding: '5px 14px', background: '#00D4AA', border: 'none', borderRadius: '8px', color: '#0B0F14', cursor: 'pointer', fontSize: '12px', fontWeight: 600 }}>Today</button>
          </div>
        </div>
            ...
            {calendarViewType === 'week' ? (
              <div ref={calendarScrollRef} style={{ flex: 1, overflow: 'auto' }}>
                ...
                <div className="availability-cal" style={{ display: 'flex', flexDirection: 'column', minHeight: 'max-content' }}>
                  {/* Header Row */}
                  <div className="availability-cal-row" style={{ borderBottom: '2px solid var(--line)', position: 'sticky', top: 0, zIndex: 10, background: '#151D28' }}>
                    <div className="availability-dayhead" style={{ background: '#151D28' }}>TIME</div>
                    {visibleDates.map((date, idx) => (
                      <div 
                        key={`header-${idx}`} 
                        className={`availability-dayhead ${isToday(date) ? 'today-header' : ''}`}
                        style={{ background: isToday(date) ? undefined : '#151D28' }}
                      >
                  ...
                  {timeSlots.map((time, timeIndex) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="availability-time" style={isCurrent ? { color: '#00D4AA', fontWeight: 700 } : {}}>
                          {formatTime(time)}
                              ...
                              {isAvailable ? (
                                ...
                                <div className={`availability-event blocked ${appointment.visit_type || 'video'}`} style={{ position: 'relative' }}>
                                  {appointment.chart_locked && (
                                    <span style={{ position: 'absolute', top: 3, right: 5, fontSize: '9px', color: '#F59E0B' }} title="Chart locked">
                                      <svg width="10" height="10" viewBox="0 0 24 24" fill="currentColor"><path d="M18 8h-1V6c0-2.76-2.24-5-5-5S7 3.24 7 6v2H6c-1.1 0-2 .9-2 2v10c0 1.1.9 2 2 2h12c1.1 0 2-.9 2-2V10c0-1.1-.9-2-2-2zM12 17c-1.1 0-2-.9-2-2s.9-2 2-2 2 .9 2 2-.9 2-2 2zM9 8V6c0-1.66 1.34-3 3-3s3 1.34 3 3v2H9z"/></svg>
                                    </span>
                                  )}
                                  {appointment.status === 'completed' && !appointment.chart_locked && (
                                    <span style={{ position: 'absolute', top: 3, right: 5, fontSize: '9px', color: '#22C55E' }} title="Completed">✓</span>
                                  )}
