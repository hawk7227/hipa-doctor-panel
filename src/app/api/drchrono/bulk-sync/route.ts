    ...
    for (const p of results) {
      try {
        const record = {
          ...
          email: p.email || null,
          phone: p.cell_phone || p.home_phone || null,
          address: p.address ? `${p.address}, ${p.city || ''}, ${p.state || ''} ${p.zip_code || ''}`.trim() : null,
          city: p.city || null,
          state: p.state || null,
          ...
          primary_care_physician: p.primary_care_physician || null,
          ...
          preferred_pharmacy: p.default_pharmacy ? String(p.default_pharmacy) : null,
          raw_data: p,
        ...
        const { error } = await supabaseAdmin
          ...
          .upsert(record, { onConflict: 'drchrono_id' })
