...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await db.from('care_plans').insert({ patient_id, title, description: description||null, goals: goals||null, status: status||'active', start_date: start_date||null, end_date: end_date||null }).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
...
export async function PUT(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await db.from('care_plans').update(updates).eq('id', id).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ data })
  } catch (err: any) { return NextResponse.json({ error: err.message }, { status: 500 }) }
}
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: 'id required' }, { status: 400 })
  const { error } = await db.from('care_plans').delete().eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
