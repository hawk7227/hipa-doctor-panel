...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data: local } = await db.from('patient_clinical_notes').select('*').eq('patient_id', patient_id).order('created_at', { ascending: false }).limit(50)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await db.from('patient_clinical_notes').insert({ patient_id, appointment_id: appointment_id||null, note_type: note_type||'progress', content, author: author||null }).select().single()
...
export async function DELETE(req: NextRequest) {
  ...
  const { error } = await db.from('patient_clinical_notes').delete().eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
