...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data, error } = await db.from('chart_amendments').select('*').eq('patient_id', patient_id).order('created_at', { ascending: false }).limit(50)
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await db.from('chart_amendments').insert({
...
export async function PATCH(req: NextRequest) {
  try {
    ...
    const { data, error } = await db.from('chart_amendments').update(update).eq('id', id).select().single()
    if (error) throw error
    return NextResponse.json({ data })
  } catch (err: any) { return NextResponse.json({ error: err.message }, { status: 500 }) }
}
