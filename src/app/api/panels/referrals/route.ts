...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data, error } = await db.from('referrals').select('*').eq('patient_id', patient_id).order('created_at', { ascending: false }).limit(50)
...
export async function PATCH(req: NextRequest) {
  ...
  try {
    ...
    const { data, error } = await db.from('referrals').update(update).eq('id', id).select().single()
    if (error) throw error
    return NextResponse.json({ data })
  } catch (err: any) { return NextResponse.json({ error: err.message }, { status: 500 }) }
}
...
// ═══ BUILD_HISTORY ═══════════════════════════════════════════
// This file: Panel API for referrals
...
//
// WIRING: Called by usePanelData hook from referrals panel component
