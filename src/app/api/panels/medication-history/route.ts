...
export async function GET(req: NextRequest) {
  ...
  try {
    const { data: local } = await db.from('patient_medication_history').select('*').eq('patient_id', patient_id).order('start_date', { ascending: false })
...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await db.from('patient_medication_history').insert(body).select().single()
    if (error) return NextResponse.json({ error: error.message }, { status: 500 })
    return NextResponse.json({ data })
  } catch (err: any) { return NextResponse.json({ error: err.message }, { status: 500 }) }
}

export async function PUT(req: NextRequest) { return NextResponse.json({ error: 'History records are immutable' }, { status: 405 }) }

export async function DELETE(req: NextRequest) {
  const id = req.nextUrl.searchParams.get('id')
  if (!id) return NextResponse.json({ error: 'id required' }, { status: 400 })
  const { error } = await db.from('patient_medication_history').delete().eq('id', id)
  if (error) return NextResponse.json({ error: error.message }, { status: 500 })
