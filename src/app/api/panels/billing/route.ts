...
export async function GET(req: NextRequest) {
  ...
  try {
    if (type === 'payments') {
      const { data } = await db.from('patient_payments').select('*').eq('patient_id', patient_id).order('payment_date', { ascending: false })
...
export async function POST(req: NextRequest) {
  try {
    ...
    const table = body._type === 'payment' ? 'patient_payments'
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const table = _type === 'payment' ? 'patient_payments'
...
export async function DELETE(req: NextRequest) {
  ...
  if (!id) return NextResponse.json({ error: 'id required' }, { status: 400 })
  const table = type === 'payment' ? 'patient_payments'
