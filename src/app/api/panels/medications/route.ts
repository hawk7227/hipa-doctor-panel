...
export async function POST(req: NextRequest) {
  try {
    ...
    const { patient_id, name, dosage, frequency, route, status, prescribed_date, notes } = body
    if (!patient_id || !name) return NextResponse.json({ error: 'patient_id and name required' }, { status: 400 })

    const { data, error } = await db.from('patient_medications').insert({
      patient_id, name, dosage: dosage || null, frequency: frequency || null,
      route: route || null, status: status || 'active',
      prescribed_date: prescribed_date || null, notes: notes || null,
    }).select().single()
