...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Check if patient already exists - match by drchrono_patient_id first
    ...
    if (!existingPatient && drchronoPatient.email) {
      ...
      if (data && data.length === 1) {
        ...
        existingPatient = data.find(function(p) { return p.first_name && p.last_name && p.first_name.toLowerCase() === (drchronoPatient.first_name || "").toLowerCase() && p.last_name.toLowerCase() === (drchronoPatient.last_name || "").toLowerCase() }) || null
      }
    }
