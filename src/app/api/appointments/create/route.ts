  ...
  try {
    ...
    if (!finalUserId && patientEmail) {
      ...
      try {
        ...
        if (!userError && user) {
          ...
          console.log('ℹ️ No user found with email:', patientEmail, '- appointment will be created without user_id')
        }
      ...
      console.log('ℹ️ No patient email or user_id provided - appointment will be created without user_id')
    }
    ...
    if (patientEmail || patientPhone) {
      ...
      try {
        ...
        console.warn('⚠️ Error looking up/creating patient:', patientLookupError)
    ...
    if (year !== undefined && month !== undefined && day !== undefined && hours !== undefined && minutes !== undefined) {
      ...
      const getTestValue = (type: string) => testParts.find(part => part.type === type)?.value || '0'
      ...
      const finalUTC = new Date(Date.UTC(year, month - 1, day, hours + tzOffsetHours, minutes, 0))
      
      // Verify it's correct
      const verifyParts = formatter.formatToParts(finalUTC)
      const getVerifyValue = (type: string) => verifyParts.find(part => part.type === type)?.value || '0'
      ...
      if (verifyDay !== day || verifyHour !== hours || verifyMinute !== minutes) {
    ...
    if (!doctorId) {
      console.error('❌ ERROR: doctorId is null or undefined! Cannot create appointment without doctor_id.')
      return NextResponse.json(
    ...
    try {
      const appointmentDate = requestedDateTimeISO ? new Date(requestedDateTimeISO).toLocaleString() : 'Date to be confirmed'
      ...
      console.warn('⚠️ Failed to send admin notification for appointment:', notificationError)
