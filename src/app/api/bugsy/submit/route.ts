...
// Version: 2.3.0 — Video upload to Supabase Storage
// ============================================================================

import { NextRequest, NextResponse } from 'next/server';
import { createClient } from '@supabase/supabase-js';

// Increase timeout for video upload processing
...
export async function POST(request: NextRequest) {
  try {
    ...
    const { report, priority, priority_signals, video_base64 } = body;
    ...
    // ── Upload video from base64 to Supabase Storage ──
    let finalVideoUrl: string | null = null;
    if (video_base64 && typeof video_base64 === 'string' && video_base64.startsWith('data:')) {
      try {
        console.log('Bugsy: Uploading video to storage...');
        const base64Data = video_base64.split(',')[1];
        const buffer = Buffer.from(base64Data, 'base64');
        const fileName = `recordings/${doctorId}/${Date.now()}-recording.webm`;
        ...
        if (uploadError) {
          console.error('Bugsy: Storage upload error:', uploadError);
        } else if (uploadData) {
          const { data: urlData } = supabase.storage
            .from('bug-reports')
            .getPublicUrl(uploadData.path);
          finalVideoUrl = urlData.publicUrl;
          console.log('Bugsy: Video uploaded to:', finalVideoUrl);
        }
      } catch (storageErr) {
        console.error('Bugsy: Video upload failed:', storageErr);
      }
    }

    // ── Clean attachments — replace blob URLs with uploaded storage URL ──
      ...
      cleanAttachments = cleanAttachments
        .map((att: any) => {
          ...
          if (cleaned.url && cleaned.url.startsWith('blob:')) {
            cleaned.url = finalVideoUrl || null;
          }
          return cleaned;
        })
    ...
    const bugReportData: Record<string, any> = {
      ...
      session_id: null, // Bugsy session IDs are not UUIDs — skip this field
      ...
      // Recording — use uploaded storage URL
