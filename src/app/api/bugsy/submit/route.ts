...
export async function POST(request: NextRequest) {
  try {
    ...
    // ── Recording URL — client uploads to storage, sends real URL ──
    let recordingUrl = report.recording_url || null;
    if (recordingUrl && recordingUrl.startsWith('blob:')) {
      recordingUrl = null; // safety fallback — should never happen now
    }

    // ── Clean attachments — strip any remaining blob URLs ──
    ...
    try {
      const { pushToRole } = await import('@/lib/pushNotify');
      await pushToRole('admin', {
        title: `🐛 New Bug Report from ${reporterName}`,
        body: title.slice(0, 100),
        url: '/admin/bug-reports',
        type: 'bug_report',
        tag: `bug-${insertedReport?.id}`,
      });
    } catch (pushErr) {
