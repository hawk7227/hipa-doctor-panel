...
export default function BugsyModal({ isOpen, onClose, onSubmitSuccess }: BugsyModalProps) {
  ...
  // Handle processing phase — SKIP analysis/reflection/clarifying, go to verification
  const skipProcessedRef = useRef(false);

  useEffect(() => {
    const phase = bugsy.phase;

    if (phase === 'idle' || phase === 'recording') {
      skipProcessedRef.current = false; // reset for next recording
      return;
    }

    if (phase === 'processing' && !skipProcessedRef.current) {
      skipProcessedRef.current = true;
      // Create minimal analysis and set it — this moves phase to 'reflection'
      bugsy.setAnalysis({
    ...
    if (phase === 'reflection') {
      // Auto-confirm — this moves to 'clarifying' or 'verification'
      bugsy.confirmReflection(true);
      return;
    }

    if (phase === 'clarifying') {
      // Force skip — this moves to 'verification'
