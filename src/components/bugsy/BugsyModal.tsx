...
export default function BugsyModal({ isOpen, onClose, onSubmitSuccess }: BugsyModalProps) {
  ...
  // Handle processing phase — SKIP analysis, go straight to verification
  useEffect(() => {
    if (bugsy.phase === 'processing') {
      // Skip analysis, reflection, clarifying — go straight to verification
      // Create a minimal analysis from the transcript
      const quickAnalysis: AnalysisResult = {
      ...
      // Set analysis (moves to reflection) then immediately skip to verification
      bugsy.setAnalysis(quickAnalysis);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bugsy.phase]);

  // Skip reflection and clarifying — jump to verification
  useEffect(() => {
    if (bugsy.phase === 'reflection' || bugsy.phase === 'clarifying') {
      // Auto-confirm and skip to verification
      bugsy.confirmReflection(true);
      // If still in clarifying, force to verification
      if (bugsy.phase === 'clarifying') {
        bugsy.setAnswers(bugsy.answers);
      }
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bugsy.phase]);
