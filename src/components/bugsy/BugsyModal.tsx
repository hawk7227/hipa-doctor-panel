...
import type { BugsyModalProps } from '@/types/bugsy';
      ...
      const runAnalysis = async () => {
        try {
          ...
          const mockAnalysis = {
            ...
            steps_to_reproduce: bugsy.markers.map((m, i) => `Step ${i + 1}: Clicked on ${m.element_tag || 'element'}`),
            priority_suggestion: 'medium' as const,
            pattern_matches: [] as Array<{ pattern_id: string; pattern_name: string; confidence: number }>,
            confidence: 0.7,
            files_identified: [] as string[],
            api_endpoints_identified: [] as string[],
            similar_bugs: [] as Array<{ bug_id: string; title: string; similarity: number }>,
          };
