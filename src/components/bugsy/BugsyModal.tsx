...
export default function BugsyModal({ isOpen, onClose, onSubmitSuccess }: BugsyModalProps) {
  ...
  // When analysis is set and phase moves to reflection, skip straight to verification
  useEffect(() => {
    if (bugsy.phase === 'reflection') {
      // Skip reflection - auto-confirm and move forward
      bugsy.confirmReflection(true);
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [bugsy.phase]);

  // When phase moves to clarifying, skip straight through
  useEffect(() => {
    if (bugsy.phase === 'clarifying') {
      // Skip clarifying questions - move to verification with current answers
  ...
  return (
    ...
    >
      ...
      >
        ...
        {/* Confidence Footer (only shown on verification screen now) */}
        {bugsy.phase === 'verification' && (
