      ...
      const initDoctor = async () => {
        try {
          ...
          if (user) {
            ...
            // Fallback: use auth user id and email
            bugsy.initializeContext(user.id, user.email || 'Doctor', 'provider');
          } else {
            console.error('Bugsy: No authenticated user found');
            bugsy.initializeContext('', 'Doctor', 'provider');
          }
        } catch (err) {
