      ...
      {videoUrl && (
        <div className="mb-6 rounded-xl overflow-hidden border border-teal-500/20 bg-black">
          <div className="relative">
            <video ref={videoRef} src={videoUrl} className="w-full max-h-[240px] object-contain bg-black" playsInline onEnded={() => setIsPlaying(false)} onPlay={() => setIsPlaying(true)} onPause={() => setIsPlaying(false)} />
            <div className="absolute bottom-0 left-0 right-0 bg-gradient-to-t from-black/80 to-transparent p-3">
              <div className="flex items-center gap-3">
                <button onClick={togglePlay} className="p-2 bg-white/20 hover:bg-white/30 rounded-full">{isPlaying ? <Pause className="w-4 h-4 text-white" /> : <Play className="w-4 h-4 text-white" />}</button>
                <button onClick={restart} className="p-2 bg-white/20 hover:bg-white/30 rounded-full"><SkipBack className="w-3.5 h-3.5 text-white" /></button>
                <button onClick={toggleMute} className="p-2 bg-white/20 hover:bg-white/30 rounded-full">{isMuted ? <VolumeX className="w-4 h-4 text-white" /> : <Volume2 className="w-4 h-4 text-white" />}</button>
                <span className="text-xs text-white/60 ml-auto">Screen Recording</span>
              </div>
            </div>
          </div>
        </div>
