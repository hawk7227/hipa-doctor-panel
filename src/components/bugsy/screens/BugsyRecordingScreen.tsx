...
// Version: 1.1.1 - Fixed stale closure on duration + speech recognition restart
// ============================================================================
...
export default function BugsyRecordingScreen({ onStop, context }: BugsyRecordingScreenProps) {
  ...
  const markersRef = useRef<ScreenMarker[]>([]);
  ...
  const handleStopRecording = useCallback(() => {
    // Use ref for current duration (avoids stale closure)
    if (durationRef.current < MIN_DURATION_SECONDS) {
  ...
  const handleRecordingComplete = () => {
    ...
    const transcriptData: TranscriptData = {
      ...
      keywords_found: extractKeywords(currentTranscript),
    };

    // Use refs for current state values
    onStop(recordingData, transcriptData, interactionsRef.current, markersRef.current);
    stopAllStreams();
  };
