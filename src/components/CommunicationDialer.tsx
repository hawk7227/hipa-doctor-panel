  ...
  const saveCallHistory = async (call: Call, formattedNumber: string) => {
    try {
      ...
      const saveFn = async (sid: string, status: string) => {
        ...
        if (existing) {
          await supabase.from('communication_history').update({ status, ...(patientId ? { patient_id: patientId } : {}) })
  ...
  const handleSendSMS = async () => {
    ...
    try {
      ...
      const res = await fetch('/api/communication/sms', {
        ...
        credentials: 'include',
        body: JSON.stringify({ to: formatted, message: smsMessage.trim(), patientId })
      })
