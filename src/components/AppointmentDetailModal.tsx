...
export default function AppointmentDetailModal({ 
  ...
  const handleSignAndLockChart = useCallback(async () => {
    ...
    try {
      ...
      setAppointment((prev: any) => prev ? { ...prev, chart_locked: true, is_locked: true, chart_status: 'signed', chart_signed_at: now, chart_signed_by: doctorName } : prev)
      onStatusChange()
    } catch (err: any) {
          ...
          <div className="sticky top-0 bg-slate-900/95 backdrop-blur-md border-b border-white/10 z-10 flex-shrink-0 px-4 py-3">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-bold text-sm sm:text-base">
                ...
                {/* Unlock button — only when chart is signed or has addendums */}
                {(chartStatus === 'signed' || chartStatus === 'addendum') && (
                  <button
                    onClick={() => {
                      if (confirm('Unlock this chart? This will revert to Draft status and make all fields editable again.')) {
                        handleUnlockChart()
                      }
                    }}
                    ...
                    title="Unlock chart — revert to draft"
                  >
                    <Unlock className="h-3 w-3" />Unlock
                  </button>
                )}

                {/* Add Addendum button — only when chart is signed or has addendums */}
                {(chartStatus === 'signed' || chartStatus === 'addendum') && (
                  <button
                    ...
                    title="Add addendum to signed chart"
                  >
