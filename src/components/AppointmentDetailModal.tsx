...
import { X, Edit, Save, Calendar, Clock, CheckCircle, XCircle, ArrowRight, RotateCcw, Pill, FileText, ClipboardList, CalendarDays, AlertTriangle, AlertCircle, Activity, Mic, Phone, ExternalLink, Lock, Unlock, Stethoscope, User, FlaskConical, Syringe, FolderOpen, Users, Wine, Scissors, Building2, ClipboardCheck, DollarSign, MessageSquare, Video } from 'lucide-react'
          ...
          return (
            <div key={sectionId} {...sectionProps}>
              ...
              {!layout.isCustomizeMode && appointment && (
                <div className="border border-white/10 rounded-xl overflow-hidden" style={{ background: '#0d1424' }}>

                  {/* ─── Patient Intake ─── */}
                  <div className="px-4 py-3 border-b border-white/5">
                    <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-2">Patient Intake</div>

                    {/* Reason for Visit */}
                    <div className="rounded-lg border border-white/5 p-3 mb-3" style={{ background: 'rgba(255,255,255,0.02)' }}>
                      <div className="text-[9px] text-gray-500 uppercase font-bold tracking-wider mb-1">Reason For Visit / Symptoms</div>
                      <div className="text-sm text-emerald-400 font-medium">
                        {(appointment as any).reason_for_visit || soapNotes?.chiefComplaint || (appointment as any).chief_complaint || 'Not specified'}
                      </div>
                    </div>

                    {/* Intake Form Answers — fetched from booking intake + patient records */}
                    <div className="rounded-lg border border-white/5 p-3 mb-3" style={{ background: 'rgba(255,255,255,0.02)' }}>
                      <div className="text-[9px] text-gray-500 uppercase font-bold tracking-wider mb-2">Intake Form Answers</div>
                      <div className="grid grid-cols-3 gap-3">
                        <div>
                          <div className="text-[9px] text-gray-500 mb-0.5">Drug Allergies</div>
                          <div className="text-xs font-bold" style={{ color: (appointment as any).intake_drug_allergies || ((appointment as any).patients?.allergies && (appointment as any).patients.allergies.length > 0) ? '#f87171' : '#4ade80' }}>
                            {(appointment as any).intake_drug_allergies || ((appointment as any).patients?.allergies && (appointment as any).patients.allergies.length > 0 ? 'Yes' : 'No')}
                          </div>
                        </div>
                        <div>
                          <div className="text-[9px] text-gray-500 mb-0.5">Ongoing Medical Issues</div>
                          <div className="text-xs font-bold" style={{ color: (appointment as any).intake_ongoing_issues || ((appointment as any).patients?.problems && (appointment as any).patients.problems.length > 0) ? '#f87171' : '#4ade80' }}>
                            {(appointment as any).intake_ongoing_issues || ((appointment as any).patients?.problems && (appointment as any).patients.problems.length > 0 ? 'Yes' : 'No')}
                          </div>
                        </div>
                        <div>
                          <div className="text-[9px] text-gray-500 mb-0.5">Recent Surgeries</div>
                          <div className="text-xs font-bold" style={{ color: (appointment as any).intake_recent_surgeries ? '#f87171' : '#4ade80' }}>
                            {(appointment as any).intake_recent_surgeries || 'No'}
                          </div>
                        </div>
                      </div>
                    </div>

                    {/* Allergies detail (from patient record) */}
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <div className="text-[9px] text-gray-500 uppercase font-bold tracking-wider mb-1">Allergies</div>
                        <div className="text-xs text-white/70">
                          {(appointment as any).patients?.allergies_display
                            || ((appointment as any).patients?.allergies && Array.isArray((appointment as any).patients.allergies)
                              ? (appointment as any).patients.allergies.map((a: any) => typeof a === 'string' ? a : a.allergy_name || a.name).join(', ')
                              : 'Unknown')}
                        </div>
                      </div>
                      <div>
                        <div className="text-[9px] text-gray-500 uppercase font-bold tracking-wider mb-1">Ongoing Medical Issues</div>
                        <div className="text-xs text-white/70">
                          {(appointment as any).patients?.problems_display
                            || ((appointment as any).patients?.problems && Array.isArray((appointment as any).patients.problems)
                              ? (appointment as any).patients.problems.map((p: any) => typeof p === 'string' ? p : p.problem_name || p.name || p.description).join(', ')
                              : 'None reported')}
                        </div>
                      </div>
                    </div>
                  ...
                  {/* ─── SOAP Notes (from Medazon Scribe) ─── */}
                  <div className="max-h-[300px] overflow-y-auto overscroll-contain px-3 py-3" style={{ WebkitOverflowScrolling: 'touch' as any }}>
                      ...
                      {([
                        ...
                        <div key={item.label} className="rounded-lg border border-white/5 p-2" style={{ background: 'rgba(255,255,255,0.02)' }}>
                          ...
                          <div className="text-[11px] text-gray-300 whitespace-pre-wrap leading-relaxed min-h-[16px]">
                            {item.value || <span className="text-gray-600 italic">Waiting for Medazon Scribe...</span>}
                          </div>
                        </div>
                      ))}
                    </div>
        ...
        case 'erx-composer':
          return (
            <ErxComposer
              ...
              onEditingRxDataChange={(data: any) => { prescriptions.setEditingRxData(data) }}
              onSendERx={async () => {
          ...
          return (
            <div key={sectionId} {...sectionProps}>
              ...
              <EnhancedSMSPanel
                ...
                onSendSMS={async (to: string, message: string) => {
