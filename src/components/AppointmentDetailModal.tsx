        ...
        case 'meeting-info':
          return (
            <div key={sectionId} {...sectionProps}>
              ...
              {/* ─── Tool toggle buttons (below video) ─── */}
              {!layout.isCustomizeMode && (
                <div className="mt-3 flex gap-2">
                  <button onClick={() => { setShowFreedPanel(!showFreedPanel); if (!showFreedPanel) setShowDialerPanel(false) }}
                    className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-all border ${showFreedPanel ? 'border-emerald-400 bg-emerald-500/20 text-emerald-300' : 'border-white/10 bg-white/5 text-slate-300 hover:border-emerald-400/50 hover:text-emerald-300'}`}>
                    <Mic className="h-3.5 w-3.5" style={{ color: '#10b981' }} />AI Scribe
                  </button>
                  <button onClick={() => { setShowDialerPanel(!showDialerPanel); if (!showDialerPanel) setShowFreedPanel(false) }}
                    className={`flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-xs font-bold transition-all border ${showDialerPanel ? 'border-sky-400 bg-sky-500/20 text-sky-300' : 'border-white/10 bg-white/5 text-slate-300 hover:border-sky-400/50 hover:text-sky-300'}`}>
                    <Phone className="h-3.5 w-3.5" style={{ color: '#0ea5e9' }} />Dialer
                    {appointment?.patients?.phone && <span className="text-[10px] opacity-60 ml-1">({appointment.patients.phone})</span>}
                  </button>
                </div>
              )}

              {/* ─── Freed AI Scribe Panel ─── */}
              {showFreedPanel && (
                <div className="mt-3 rounded-xl border border-emerald-500/30 overflow-hidden" style={{ background: 'linear-gradient(135deg, #064e3b, #0d1117)' }}>
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-emerald-500/20">
                    <div className="flex items-center gap-2">
                      <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                      <span className="text-sm font-bold text-emerald-300">Freed AI Scribe</span>
                      <span className="text-[10px] text-emerald-500/70">Powered by Deepgram</span>
                    </div>
                    <button onClick={() => setShowFreedPanel(false)} className="p-1 rounded hover:bg-white/10">
                      <X className="h-3.5 w-3.5 text-gray-500 hover:text-red-400" />
                    </button>
                  </div>
                  <div className="p-4 space-y-3">
                    <p className="text-xs text-gray-400">Open Freed to capture this conversation and auto-generate SOAP notes. Log in or sign up for a free 7-day trial.</p>
                    <a href="https://secure.getfreed.ai" target="_blank" rel="noopener noreferrer"
                      className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-bold text-sm bg-emerald-600 hover:bg-emerald-500 text-white transition-all">
                      <ExternalLink className="h-4 w-4" />Open Freed Web App
                    </a>
                    <a href="https://chromewebstore.google.com/detail/freed/fbemknilbghlokgjhomnikdkljnbpgmo" target="_blank" rel="noopener noreferrer"
                      className="w-full flex items-center justify-center gap-2 px-4 py-2 rounded-lg font-bold text-xs border border-emerald-500/30 hover:border-emerald-400 text-emerald-400 hover:text-emerald-300 transition-all">
                      Get Chrome Extension (EHR Push)
                    </a>
                    <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider pt-1">Download on your phone</p>
                    <div className="flex gap-2">
                      <a href="https://apps.apple.com/us/app/freed-ai-medical-scribe/id6449428266" target="_blank" rel="noopener noreferrer"
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold bg-black/40 border border-white/10 hover:border-white/30 text-white transition-all">
                        🍎 App Store
                      </a>
                      <a href="https://play.google.com/store/apps/details?id=ai.getfreed.freed" target="_blank" rel="noopener noreferrer"
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold bg-black/40 border border-white/10 hover:border-white/30 text-white transition-all">
                        ▶️ Google Play
                      </a>
                    </div>
                    <p className="text-[10px] text-gray-600 text-center">Free 7-day trial • $99/mo • HIPAA · SOC2 · HITECH compliant</p>
                  </div>
                </div>
              )}

              {/* ─── Grasshopper Dialer Panel ─── */}
              {showDialerPanel && (
                <div className="mt-3 rounded-xl border border-sky-500/30 overflow-hidden" style={{ background: 'linear-gradient(135deg, #0c2d48, #0d1117)' }}>
                  <div className="flex items-center justify-between px-4 py-2.5 border-b border-sky-500/20">
                    <div className="flex items-center gap-2">
                      <Phone className="h-4 w-4 text-sky-400" />
                      <span className="text-sm font-bold text-sky-300">Grasshopper Dialer</span>
                      <span className="text-[10px] text-sky-500/70">VoIP</span>
                    </div>
                    <button onClick={() => setShowDialerPanel(false)} className="p-1 rounded hover:bg-white/10">
                      <X className="h-3.5 w-3.5 text-gray-500 hover:text-red-400" />
                    </button>
                  </div>
                  <div className="p-4 space-y-3">
                    {appointment?.patients?.phone && (
                      <div className="flex items-center gap-2 p-2.5 rounded-lg border border-sky-500/20" style={{ background: 'rgba(14,165,233,0.08)' }}>
                        <Phone className="h-4 w-4 text-sky-400 flex-shrink-0" />
                        <span className="text-sm font-mono text-white font-bold flex-1">{appointment.patients.phone}</span>
                        <button onClick={() => navigator.clipboard?.writeText(appointment?.patients?.phone || '')}
                          className="px-2 py-1 rounded text-[10px] font-bold text-sky-400 hover:text-sky-300 border border-sky-500/30 hover:border-sky-400 transition-all">Copy</button>
                        <a href={`tel:${appointment.patients.phone}`}
                          className="px-2 py-1 rounded text-[10px] font-bold text-green-400 hover:text-green-300 border border-green-500/30 hover:border-green-400 transition-all flex items-center gap-1">
                          <Phone className="h-3 w-3" />Call
                        </a>
                      </div>
                    )}
                    <a href="https://app.grasshopper.com" target="_blank" rel="noopener noreferrer"
                      className="w-full flex items-center justify-center gap-2 px-4 py-2.5 rounded-lg font-bold text-sm bg-sky-600 hover:bg-sky-500 text-white transition-all">
                      <ExternalLink className="h-4 w-4" />Open Grasshopper Web App
                    </a>
                    <p className="text-[10px] text-gray-500 uppercase font-bold tracking-wider pt-1">Download on your phone</p>
                    <div className="flex gap-2">
                      <a href="https://apps.apple.com/us/app/grasshopper/id579356813" target="_blank" rel="noopener noreferrer"
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold bg-black/40 border border-white/10 hover:border-white/30 text-white transition-all">
                        🍎 App Store
                      </a>
                      <a href="https://play.google.com/store/apps/details?id=com.grasshopper.dialer" target="_blank" rel="noopener noreferrer"
                        className="flex-1 flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold bg-black/40 border border-white/10 hover:border-white/30 text-white transition-all">
                        ▶️ Google Play
                      </a>
                    </div>
                    <a href="https://grasshopper.com/apps/" target="_blank" rel="noopener noreferrer"
                      className="w-full flex items-center justify-center gap-1.5 px-3 py-1.5 rounded-lg text-[11px] font-bold border border-white/10 hover:border-sky-400/50 text-gray-400 hover:text-sky-300 transition-all">
                      Desktop App (Mac / Windows)
                    </a>
                    <p className="text-[10px] text-gray-600 text-center">Free 7-day trial • Starts at $14/mo • VoIP + WiFi calling</p>
                  </div>
                </div>
              )}
            </div>
