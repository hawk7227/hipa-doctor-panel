    ...
    <>
      ...
      {isMinimized ? (
        ...
        isMobile ? (
          ...
          <div className="fixed bottom-0 left-0 right-0 z-50" style={{ background: 'linear-gradient(90deg, #0a1628, #0f172a, #0a1628)', borderTop: '1px solid rgba(0, 230, 255, 0.2)', boxShadow: '0 -4px 24px rgba(0,0,0,0.5)' }}>
          ...
          /* ── Desktop minimized: full bottom bar with EHR shortcuts (UNCHANGED) ── */
              ...
              <div className="absolute top-2 right-2 z-20 cursor-pointer overflow-hidden flex items-center justify-center"
                ...
                {drSelfViewExpanded ? (
                  <div style={{ textAlign: 'center' }}>
                    <div className="text-[10px] text-slate-400 font-semibold">{appointment?.providers?.full_name || appointment?.providers?.first_name ? `Dr. ${appointment.providers.last_name || appointment.providers.first_name}` : 'Dr.'}</div>
                  </div>
                ) : (
                  <div className="w-7 h-7 rounded flex items-center justify-center text-[10px] font-extrabold text-white" style={{ background: 'linear-gradient(135deg, #0e7490, #06b6d4)' }}>
                    {(appointment?.providers?.first_name?.[0] || 'D')}{(appointment?.providers?.last_name?.[0] || 'H')}
                  </div>
                )}
              </div>
            )}
                          ...
                          {drSelfViewExpanded ? (
                            <div style={{ textAlign: 'center' }}>
                              <div className="text-[11px] text-slate-400 font-semibold">
                                {appointment?.providers?.full_name || appointment?.providers?.first_name ? `Dr. ${appointment.providers.last_name || appointment.providers.first_name}` : 'Dr.'}
                              </div>
                    ...
                    <div style={{ borderTop: '1px solid #1e293b', background: 'linear-gradient(180deg, #0d1424, #0a1018)', flexShrink: 0, maxHeight: patientCardCollapsed ? '40px' : '45%', transition: 'max-height 0.3s ease', overflow: 'hidden' }}>
                      ...
                      <button onClick={() => setPatientCardCollapsed(!patientCardCollapsed)}
                        ...
                        <span className="flex items-center gap-2">
                          ...
                          {/* Allergy alert badge */}
                          {problemsMedications.allergies && (problemsMedications.allergies as any[]).length > 0 && (
