    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full z-50 flex" style={{ left: panelWidth ? `calc(100vw - ${panelWidth}px)` : 'var(--sidebar-width, 240px)' }}>
        ...
        <div className={`flex-1 h-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-l border-white/20 shadow-2xl transform transition-transform duration-300 ${
          ...
          {confirmDialog.open && (
            <div className="fixed inset-0 z-[99999] flex items-center justify-center" onClick={() => setConfirmDialog(prev => ({ ...prev, open: false }))}>
              <div className="absolute inset-0 bg-black/50 backdrop-blur-[2px]" />
              <div className="relative bg-slate-800 rounded-xl border border-white/10 shadow-2xl w-[340px] overflow-hidden" onClick={(e) => e.stopPropagation()}>
                {/* Top accent bar */}
                <div className={`h-1 w-full ${
                  confirmDialog.icon === 'danger' ? 'bg-red-500' :
                  confirmDialog.icon === 'warning' ? 'bg-amber-500' :
                  confirmDialog.icon === 'success' ? 'bg-green-500' : 'bg-blue-500'
                }`} />
                <div className="p-5">
                  {/* Icon + Title */}
                  <div className="flex items-start gap-3 mb-3">
                    <div className={`flex-shrink-0 w-9 h-9 rounded-lg flex items-center justify-center ${
                      confirmDialog.icon === 'danger' ? 'bg-red-500/15' :
                      confirmDialog.icon === 'warning' ? 'bg-amber-500/15' :
                      confirmDialog.icon === 'success' ? 'bg-green-500/15' : 'bg-blue-500/15'
                    }`}>
                      <AlertTriangle className={`h-5 w-5 ${
                        confirmDialog.icon === 'danger' ? 'text-red-400' :
                        confirmDialog.icon === 'warning' ? 'text-amber-400' :
                        confirmDialog.icon === 'success' ? 'text-green-400' : 'text-blue-400'
                      }`} />
                    </div>
                    <div>
                      <h3 className="text-sm font-bold text-white leading-tight">{confirmDialog.title}</h3>
                      <p className="text-xs text-gray-400 mt-1.5 leading-relaxed">{confirmDialog.message}</p>
                    </div>
                  </div>
                  {/* Buttons */}
                  <div className="flex gap-2 justify-end mt-4">
                    <button
                      ...
                      className="px-3.5 py-1.5 rounded-lg text-xs font-semibold bg-white/5 text-gray-300 border border-white/10 hover:bg-white/10 hover:text-white transition-all"
                    >
                    ...
                    <button
                      ...
                      className={`px-3.5 py-1.5 rounded-lg text-xs font-bold text-white transition-all ${
                        confirmDialog.icon === 'danger' ? 'bg-red-600 hover:bg-red-700' :
                        confirmDialog.icon === 'warning' ? 'bg-amber-600 hover:bg-amber-700' :
                        confirmDialog.icon === 'success' ? 'bg-green-600 hover:bg-green-700' : 'bg-blue-600 hover:bg-blue-700'
                      }`}
                    >
