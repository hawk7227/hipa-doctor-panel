...
import { X, Edit, Save, Calendar, Clock, CheckCircle, XCircle, ArrowRight, RotateCcw, Pill, FileText, ClipboardList, CalendarDays, AlertTriangle, AlertCircle, Activity, Mic, Phone, ExternalLink, Lock, Unlock, Stethoscope, User, FlaskConical, Syringe, FolderOpen, Users, Wine, Scissors, Building2, ClipboardCheck, DollarSign, MessageSquare } from 'lucide-react'
...
export default function AppointmentDetailModal({ 
          ...
          return (
            <div key={sectionId} {...sectionProps}>
              ...
              <DailyMeetingEmbed
                ...
                onOpenCommHub={(tab) => { setShowCommHub(true) }}
              />
                ...
                <div className="border-x border-b border-white/10 rounded-b-xl overflow-hidden" style={{ background: '#0d1424', marginTop: '-1px' }}>
                  ...
                  {appointment?.patients?.phone && (
                    <div className="flex items-center gap-2 px-3 py-2 border-b border-white/5" style={{ background: 'rgba(14,165,233,0.04)' }}>
                      ...
                      <button onClick={() => {
  const callSection = document.querySelector('[data-section-id="call-section"]')
  if (callSection) callSection.scrollIntoView({ behavior: 'smooth', block: 'center' })
}}
  ...
  <Phone className="h-3 w-3" />Call via Twilio
</button>
                    </div>
                  )}
