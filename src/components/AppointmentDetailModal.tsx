...
export default function AppointmentDetailModal({ 
  ...
  // NEW: Video controls bar — visual toggles matching prototype
  // These are UI-only toggles for the overlay controls bar.
  // When Daily.co SDK is active, its built-in controls handle the actual call.
  ...
  const [panelMode, setPanelMode] = useState<'full' | 'floating'>('floating') // floating = draggable/resizable window, full = takes whole screen
  ...
  // Portal renders the entire panel outside the dashboard's DOM tree
  // This prevents z-fighting, click-through, and style bleed from the dashboard
  const portalContent = (
    <>
      {/* Backdrop — only in full mode, floating mode lets doctor use dashboard */}
      {!isMinimized && panelMode === 'full' && (
        <div className="fixed inset-0 bg-black/40 z-[9998] transition-opacity duration-300" onClick={onClose} />
      )}
      ...
      {isMinimized ? (
        ...
        isMobile ? (
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[9999]" style={{ background: 'linear-gradient(90deg, #0a1628, #0f172a, #0a1628)', borderTop: idleAlarmFiring ? '2px solid #ef4444' : '1px solid rgba(0, 230, 255, 0.2)', boxShadow: '0 -4px 24px rgba(0,0,0,0.5)' }}>
          ...
          <div className="fixed bottom-0 left-0 right-0 z-[9999] bg-gradient-to-r from-slate-900 via-slate-800 to-slate-900 shadow-2xl" style={{ borderTop: idleAlarmFiring ? '2px solid #ef4444' : '1px solid rgba(6,182,212,0.3)' }}>
        ...
        /* ═══════════════════════════════════════════════════════════════
           ...
           ═══════════════════════════════════════════════════════════════ */
        <div className="fixed inset-0 z-[9999] flex flex-col" style={{ background: '#0a1018' }}
        ...
        <div className="fixed top-0 right-0 h-full w-full flex"
          ...
          style={{
            ...
            ...(panelMode === 'floating' ? {
              ...
              borderRadius: '12px',
              boxShadow: '0 25px 60px rgba(0,0,0,0.7), 0 0 0 1px rgba(255,255,255,0.08), 0 0 120px rgba(0,0,0,0.4)',
              transition: (isResizing || isDraggingPanel) ? 'none' : 'border-radius 0.3s ease',
              overflow: 'hidden',
              background: '#0a1018', // Solid opaque background — nothing bleeds through
            } : { background: '#0a1018' })
          }}>
                ...
                <>
                  ...
                  <div className="flex flex-col overflow-hidden" style={{ width: '45%', minWidth: '380px', borderRight: '1px solid #1e293b', transition: 'width 0.35s cubic-bezier(0.4, 0, 0.2, 1)' }}>
                    ...
                    <div className="flex-1 min-h-0 relative" style={{ background: '#000' }}>
                      ...
                      {/* ═══ VIDEO CONTROLS BAR — matches prototype layout ═══ */}
                      <div className="absolute bottom-16 left-1/2 -translate-x-1/2 z-20 flex items-center gap-3">
                        ...
                        <button onClick={() => setVideoCameraOn(!videoCameraOn)}
                          ...
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={videoCameraOn ? '#e2e8f0' : '#fca5a5'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M23 7l-7 5 7 5V7z" /><rect x="1" y="5" width="15" height="14" rx="2" ry="2" />
                            {!videoCameraOn && <line x1="1" y1="1" x2="23" y2="23" />}
                          </svg>
                        </button>
                        {/* Mic toggle */}
                        <button onClick={() => setVideoMicOn(!videoMicOn)}
                          ...
                          <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke={videoMicOn ? '#e2e8f0' : '#fca5a5'} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                            <path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z" />
                            <path d="M19 10v2a7 7 0 0 1-14 0v-2" /><line x1="12" y1="19" x2="12" y2="23" /><line x1="8" y1="23" x2="16" y2="23" />
                            {!videoMicOn && <line x1="1" y1="1" x2="23" y2="23" />}
                          </svg>
                        </button>
                        {/* Chat */}
                        <button className="w-11 h-11 rounded-full flex items-center justify-center transition-all"
                          style={{ background: 'rgba(30, 41, 59, 0.9)', border: '1px solid rgba(255,255,255,0.15)', backdropFilter: 'blur(8px)' }}>
                        ...
                        {/* End call */}
                        <button className="w-11 h-11 rounded-full flex items-center justify-center transition-all"
                          style={{ background: 'rgba(239, 68, 68, 0.9)', border: '1px solid rgba(239, 68, 68, 0.5)', backdropFilter: 'blur(8px)' }}>
                      ...
                      {/* Patient name + call type label — centered below controls */}
                      <div className="absolute bottom-3 left-1/2 -translate-x-1/2 z-20 text-center">
                        <div className="text-sm font-bold text-white">{appointment?.patients?.first_name} {appointment?.patients?.last_name}</div>
