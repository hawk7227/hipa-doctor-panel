...
export default function AppointmentDetailModal({ 
  ...
  useEffect(() => {
    ...
    if (appointmentChartStatus) {
      setChartStatus(appointmentChartStatus as 'draft' | 'signed' | 'addendum')
  ...
  }, [appointmentChartLocked, appointmentChartStatus, appointmentSignedAt, appointmentSignedBy])
  ...
  const [chartStatus, setChartStatus] = useState<'draft' | 'signed' | 'addendum'>('draft')
  ...
  const [addendums, setAddendums] = useState<Array<{ id: string; text: string; created_at: string; created_by: string }>>([])
    ...
    try {
      const { error } = await supabase
        ...
        .update({
          ...
          chart_status: 'signed',
          chart_signed_at: now,
          chart_signed_by: doctorName
        })
        .eq('id', appointmentId)
      if (error) throw error
      setChartLocked(true)
      setChartStatus('signed')
      setChartSignedAt(now)
      setChartSignedBy(doctorName)
      setAppointment((prev: any) => prev ? { ...prev, chart_locked: true, chart_status: 'signed', chart_signed_at: now, chart_signed_by: doctorName } : prev)
      onStatusChange()
    } catch (err: any) {
  ...
  const handleSaveAddendum = useCallback(async () => {
    ...
    try {
      ...
      // Update chart status to addendum
      await supabase
        .from('appointments')
        .update({ chart_status: 'addendum' })
        .eq('id', appointmentId)

      if (data) setAddendums(prev => [...prev, data])
      setChartStatus('addendum')
      setAddendumText('')
      setShowAddendumForm(false)
      setAppointment((prev: any) => prev ? { ...prev, chart_status: 'addendum' } : prev)
      onStatusChange()
    } catch (err: any) {
  ...
  }, [appointmentId, currentUser, addendumText, onStatusChange, setAppointment, setError])
        ...
        case 'meeting-info':
          return (
            <div key={sectionId} {...sectionProps}>
              ...
              {appointment && (appointment as any).dailyco_meeting_url && (
                <button onClick={() => setShowVideoPanel(true)}
                  disabled={layout.isCustomizeMode}
                  className="flex items-center justify-center gap-2 w-full px-4 py-3 rounded-xl font-bold text-sm text-white transition-all hover:brightness-110 disabled:opacity-60 disabled:cursor-not-allowed"
                  style={{ background: 'linear-gradient(135deg, #0e7490, #1e40af)' }}>
                  <Video className="h-4 w-4" />Open Video Consultation
                </button>
              )}
            </div>
          )
    ...
    <>
      ...
      <div className="fixed top-0 right-0 h-full z-50 flex" style={{ left: panelWidth ? `calc(100vw - ${panelWidth}px)` : 'var(--sidebar-width, 240px)' }}>
        ...
        <div className={`flex-1 h-full bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 border-l border-white/20 shadow-2xl transform transition-transform duration-300 ${
          ...
          <div className="sticky top-0 bg-slate-900/95 backdrop-blur-md border-b border-white/10 z-10 flex-shrink-0 px-4 py-3">
            <div className="flex items-center justify-between">
              <h2 className="text-white font-bold text-sm sm:text-base">
                ...
                <button
                  onClick={() => {
                    if (chartStatus === 'draft') {
                      // Confirm sign & lock
                      if (confirm('Sign and lock this chart? SOAP notes will become read-only. You can still add addendums.')) {
                        handleSignAndLockChart()
                      }
                    } else {
                      // Toggle addendum form
                      setShowAddendumForm(v => !v)
                    }
                  }}
                  className={`ml-2 flex items-center gap-1 px-2 py-0.5 rounded text-xs font-bold transition-all ${
                    chartStatus === 'draft'
                      ? 'bg-amber-600/20 text-amber-400 border border-amber-500/40 hover:bg-amber-600/40'
                      : chartStatus === 'signed'
                      ? 'bg-green-600/20 text-green-400 border border-green-500/40 hover:bg-green-600/40'
                      : 'bg-blue-600/20 text-blue-400 border border-blue-500/40 hover:bg-blue-600/40'
                  }`}
                  title={
                    chartStatus === 'draft' ? 'Click to sign & lock chart'
                    : chartStatus === 'signed' ? `Signed by ${chartSignedBy || 'Provider'} — click to add addendum`
                    : `${addendums.length} addendum(s) — click to add another`
                  }
                >
                  {chartStatus === 'draft' ? (
                    ...
                    <><FileText className="h-3 w-3" />Addendum ({addendums.length})</>
                  )}
                </button>
              </h2>
          ...
          {appointment && (chartStatus === 'signed' || chartStatus === 'addendum') && (
              ...
              {showAddendumForm && (
                <div className="px-4 py-3 border-t border-white/5">
                  ...
                  <textarea
                    ...
                    placeholder="Add addendum to signed chart..."
                    rows={3}
                  ...
                  <div className="flex gap-2 mt-2">
                    ...
                    >
                      {savingAddendum ? <div className="animate-spin rounded-full h-3 w-3 border-b-2 border-white" /> : <Save className="h-3 w-3" />}
                      Save Addendum
                    </button>
                    <button
