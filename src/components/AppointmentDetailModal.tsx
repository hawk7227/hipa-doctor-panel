        ...
        case 'meeting-info':
          return (
            <div key={sectionId} {...sectionProps}>
              ...
              {/* ─── Video + Freed AI side-by-side layout ─── */}
              <div className="flex flex-col lg:flex-row gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <DailyMeetingEmbed
                    ...
                    sectionId={sectionId}
                  />
                  {/* Grasshopper Dialer quick-bar below video */}
                  {!layout.isCustomizeMode && appointment?.patients?.phone && (
                    <div className="mt-2 flex items-center gap-2 p-2 rounded-lg border border-sky-500/20" style={{ background: 'rgba(14,165,233,0.06)' }}>
                ...
                {/* Right: Freed AI Scribe panel (replaces old Deepgram scribe) */}
                {!layout.isCustomizeMode && (
                  <div className="w-full lg:w-[340px] xl:w-[380px] flex-shrink-0 rounded-xl border border-emerald-500/20 overflow-hidden flex flex-col" style={{ background: 'linear-gradient(180deg, #0a1a14, #0d1117)', maxHeight: 'calc(100vh - 200px)' }}>
                    {/* Freed header bar */}
                    <div className="flex items-center justify-between px-3 py-2 border-b border-emerald-500/20 flex-shrink-0" style={{ background: 'rgba(16,185,129,0.08)' }}>
                      <div className="flex items-center gap-2">
                        <div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse" />
                        <span className="text-sm font-bold text-emerald-300">Freed AI</span>
                      ...
                      <div className="flex items-center gap-1">
                        <a href="https://secure.getfreed.ai" target="_blank" rel="noopener noreferrer"
                          className="p-1 rounded hover:bg-white/10 transition-colors" title="Open Freed in new tab">
                          <ExternalLink className="h-3 w-3 text-gray-500 hover:text-emerald-400" />
                        </a>
                      </div>
                    </div>

                    {/* Scrollable content — swipe/scroll for SOAP + more */}
                    <div className="flex-1 overflow-y-auto overscroll-contain" style={{ WebkitOverflowScrolling: 'touch' }}>
                      ...
                      <div className="p-4 border-b border-white/5">
                        ...
                        <p className="text-center text-xs text-gray-400 mb-3">Open Freed to capture this conversation and auto-generate SOAP notes</p>
                        <a href="https://secure.getfreed.ai" target="_blank" rel="noopener noreferrer"
                      ...
                      {/* SOAP Notes section — doctor scrolls down to see */}
                      <div className="p-4 border-b border-white/5">
                        <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-3">SOAP Notes</div>
                        <div className="space-y-2">
                          {(['Subjective', 'Objective', 'Assessment', 'Plan'] as const).map((label) => {
                            const key = label.toLowerCase()
                            const value = key === 'subjective' ? (soapNotes.chiefComplaint || '')
                              : key === 'objective' ? (soapNotes.rosGeneral || '')
                              : key === 'assessment' ? (soapNotes.assessmentPlan?.split('\n\nPlan:')[0]?.replace('Assessment:', '')?.trim() || '')
                              : (soapNotes.assessmentPlan?.split('\n\nPlan:')[1]?.trim() || soapNotes.assessmentPlan || '')
                            return (
                              <div key={key} className="rounded-lg border border-white/5 p-2.5" style={{ background: 'rgba(255,255,255,0.02)' }}>
                                <div className="text-[10px] font-bold text-emerald-400/70 uppercase tracking-wider mb-1">{label}</div>
                                <div className="text-xs text-gray-300 whitespace-pre-wrap min-h-[20px]">
                                  {value || <span className="text-gray-600 italic">Waiting for Freed transcription...</span>}
                                </div>
                              </div>
                            )
                          })}
                      ...
                      {/* Quick actions */}
                      <div className="p-4 border-b border-white/5">
                        <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-3">Quick Actions</div>
                        <div className="grid grid-cols-2 gap-2">
                          <button onClick={() => setActiveTab('SOAP')}
                            className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold bg-white/5 border border-white/10 hover:border-emerald-400/50 text-slate-300 hover:text-emerald-300 transition-all">
                            SOAP Editor
                          </button>
                          <button onClick={() => setActiveTab('Billing')}
                            className="flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg text-[11px] font-bold bg-white/5 border border-white/10 hover:border-cyan-400/50 text-slate-300 hover:text-cyan-300 transition-all">
                            Billing Codes
                          </button>
                        </div>
                      </div>
                      ...
                      {/* Get Freed section */}
                      <div className="p-4">
                        <div className="text-[10px] text-gray-500 uppercase font-bold tracking-wider mb-3">Get Freed AI</div>
                        <a href="https://chromewebstore.google.com/detail/freed/fbemknilbghlokgjhomnikdkljnbpgmo" target="_blank" rel="noopener noreferrer"
                          className="w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg font-bold text-xs border border-emerald-500/30 hover:border-emerald-400 text-emerald-400 hover:text-emerald-300 transition-all mb-2">
                          Chrome Extension (EHR Push)
                        </a>
                        <div className="flex gap-2 mb-2">
                          <a href="https://apps.apple.com/us/app/freed-ai-medical-scribe/id6449428266" target="_blank" rel="noopener noreferrer"
                            className="flex-1 flex items-center justify-center gap-1 px-2 py-1.5 rounded-lg text-[10px] font-bold bg-black/40 border border-white/10 hover:border-white/30 text-white transition-all">
                            🍎 iOS
                          </a>
                        ...
                        <p className="text-[9px] text-gray-600 text-center">HIPAA · SOC2 · HITECH compliant</p>
                      </div>
                    </div>
                  </div>
                )}
