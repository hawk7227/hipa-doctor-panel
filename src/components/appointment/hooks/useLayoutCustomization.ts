      ...
      if (savedLayout) {
        try {
          ...
          if (parsed.left && parsed.right) {
            ...
            // Ensure meeting-info exists (default to left panel after patient-header)
            if (!leftSections.includes('meeting-info') && !rightSections.includes('meeting-info')) {
              const patientHeaderIndex = leftSections.indexOf('patient-header')
              if (patientHeaderIndex !== -1) {
                leftSections.splice(patientHeaderIndex + 1, 0, 'meeting-info')
              } else {
                leftSections = [...leftSections, 'meeting-info']
              }
            }
            // Ensure sms-section exists
            if (!leftSections.includes('sms-section') && !rightSections.includes('sms-section')) {
              ...
              if (doctorNotesIndex !== -1) {
                ...
                rightSections = [...rightSections, 'sms-section']
              }
            }
            // Ensure call-section exists
            ...
            // Ensure referrals-followup exists
